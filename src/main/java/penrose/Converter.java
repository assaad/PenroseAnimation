...
import penrose.parser.Line;
import penrose.parser.SVG;
import penrose.parser.SVGParser;
...
public class Converter {
    public static Graph convert(String svg, int width, int height, int distance) {
        ...
        try {
            SVG svgRes = SVGParser.parse(svg);
            double xMin = svgRes.minX;
            double xMax = svgRes.maxX;
            double yMin = svgRes.minY;
            double yMax = svgRes.maxY;
            ...
            for (int i = 0; i < svgRes.lines.size(); i++) {
                Line l = svgRes.lines.get(i);

                double x1 = convert(l.x1, xMin, diffX);
                double y1 = convert(l.y1, yMin, diffY);
                double x2 = convert(l.x2, xMin, diffX);
                double y2 = convert(l.y2, yMin, diffY);
                double[] xVar = new double[]{x1, y1, x2, y2};
                res.add(xVar);
                strokes.add(l.stroke);
