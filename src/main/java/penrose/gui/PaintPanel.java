...
public class PaintPanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public int getInkPanelWidth() {
        return inkPanelWidth;
    }

    public int getInkPanelHeight() {
    ...
    public PaintPanel(int f, PenroseFrame frame, int width, int height) {
    ...
    public void paintComponent(Graphics g) {
        if (canvas == null) {
            canvas = new BufferedImage(inkPanelWidth, inkPanelHeight, BufferedImage.TYPE_INT_RGB);
            graphics2D = canvas.createGraphics();
            //graphics2D.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            clear();
        }
        super.paintComponent(g);
        if (frame.penroseManager.isReady() && frame.penroseManager.animationTimer.isRunning()) {
            Graphics2D gPixel = canvas.createGraphics();
            gPixel.setColor(frame.penroseManager.stroke);
            BasicStroke bs = new BasicStroke(2);
            gPixel.setStroke(bs);
            gPixel.drawLine(frame.penroseManager.previousPixel.getX(), frame.penroseManager.previousPixel.getY(), frame.penroseManager.pixel.getX(), frame.penroseManager.pixel.getY());


            Graphics2D g2d = (Graphics2D) g.create();
            g2d.drawImage(canvas, 0, 0, this);

            if (frame.penroseManager.saveAnimation) {
                try {
                    if (frame.penroseManager.getFrameNumber() % 4 == 0) {
                        //Creating a File object
                        File dir = new File("./animation/");
                        dir.mkdir();
                        String filename = "./animation/img_" + frame.penroseManager.getFrameNumber()/4+".png";
